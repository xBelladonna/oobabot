...
class MessageQueue:
    ...
    def __init__(
        self,
        message_accumulation_period: float,
        continue_on_additional_messages: bool,
        respond_to_latest_only: bool,
        skip_in_progress_responses: bool
    ) -> None:
        self.message_accumulation_period = message_accumulation_period
        self.continue_on_additional_messages = continue_on_additional_messages
        self.respond_to_latest_only = respond_to_latest_only
        self.skip_in_progress_responses = skip_in_progress_responses
    ...
    def is_waiting(self, channel_id: int) -> bool:
        ...
        if channel_id in self.wait_tasks:
            return self.wait_tasks[channel_id].done()
        return False
...
class DiscordBot(discord.Client):
    ...
    def __init__(
        ...
        self.message_queue = MessageQueue(
            ...
            discord_settings["continue_on_additional_messages"],
            discord_settings["respond_to_latest_only"],
            discord_settings["skip_in_progress_responses"]
        )
    ...
    async def on_message_delete(self, raw_message: discord.Message) -> None:
        ...
        Checks if the deleted message is in our message queue, and removes it if so.
        """
    ...
    async def process_messages(
        ...
        # system message.
        guaranteed = self.decide_to_respond.guaranteed_responses.get(channel.id, None)
        ...
        if (
            ...
            if self.message_queue.cancel_response_task(channel.id):
                ...
                if cancelled_task:
                    ...
                    self.message_queue.remove_response_task(channel.id)
                # Clear all but the latest message from the queue for this channel
                if self.message_queue.get_queue_length(channel.id) > 1:
    ...
    async def _process_message_queue(self, channel_id: int) -> None:
        ...
        self.decide_to_respond.guaranteed_responses.pop(channel_id, None)
    ...
    async def _generate_text_response(
        ...
        if isinstance(response_channel, (discord.abc.GuildChannel, discord.Thread)):
            ...
            # DMs are more like channel-less guilds, at least in terms of the UI
            guild_name = message.channel_name
            channel_name = ""
        ...
        try:
            ...
            if self.ooba_client.can_abort_generation():
                fancy_logger.get().debug("Aborting response generation...")
